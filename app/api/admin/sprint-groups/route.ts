...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // sanitize ids: only keep valid ObjectId strings
    const validPilotIds = pilotIds
      .filter((id) => mongoose.Types.ObjectId.isValid(String(id)))
      .map(String);
    if (validPilotIds.length === 0)
      return NextResponse.json(
        { error: "No valid pilotIds provided" },
        { status: 400 },
      );
    ...
    if (!Number.isInteger(groupsCount) || groupsCount < 1) {
      return NextResponse.json(
        { error: "Invalid groupsCount" },
        { status: 400 },
      );
    }

    // validate pilot ids belong to championship
    // accept pilots that either have a top-level `championshipId` or a registration
    // entry referencing this championship (legacy/normalized records differ)
    const pilots = await Pilot.find({
      _id: { $in: validPilotIds.map((id) => id) },
      $or: [
        { championshipId: championship._id },
        { "registrations.championshipId": championship._id },
      ],
    }).lean();
    const foundPilotIds = new Set(pilots.map((p) => String(p._id)));
    const filteredPilotIds = validPilotIds.filter((id) =>
      foundPilotIds.has(id),
    );
    // remove DNS pilots from grouping (they should not be assigned to any group)
    const dnsPilotIds = Array.isArray(body.dnsPilotIds)
      ? [...new Set(body.dnsPilotIds.map(String))]
      : [];
    const validDnsPilotIds = dnsPilotIds
      .filter((id) => mongoose.Types.ObjectId.isValid(String(id)))
      .map(String);
    const dnsSet = new Set(validDnsPilotIds);
    ...
    // remove any existing groups for this stage to avoid duplicate key errors
    ...
    if (validDnsPilotIds.length > 0) {
      const stageDoc = await Stage.findById(body.stageId);
      if (stageDoc) {
        for (const pid of validDnsPilotIds) {
    ...
    // return error message for easier debugging rather than throwing
    return NextResponse.json(
